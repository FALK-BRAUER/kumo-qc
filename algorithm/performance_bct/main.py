...
class BCTPerformanceAlgorithm(QCAlgorithm):
    ...
    def initialize(self) -> None:
        ...
        if poly is not None:
            ...
            # Cloud: S&P 500 universe from ObjectStore (GH #80)
            obj_key = "sp500_universe_fy2025.json"
            if self.object_store.contains_key(obj_key):
                sp500_poly = json.loads(self.object_store.read(obj_key))
                all_tickers = sorted({t for tickers in sp500_poly.values() for t in tickers})
                self.log(f"CLOUD_UNIVERSE|object_store_sp500|unique_tickers={len(all_tickers)}")
                ...
                # Fallback if ObjectStore not populated
