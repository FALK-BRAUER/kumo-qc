...
Faithful carve of oracle _rebalance L527-590 (baseline-oracle-v0).
Reads ranked_candidates from universe phase, writes sized_orders (qty=0 stubs for sizing phase).
...
class BctScoreFull(PhaseInterface):
    PHASE_KIND = "signal"
    ...
    def evaluate(self, ctx: PhaseContext) -> PhaseResult:
        ...
        min_score = self._params.get("min_score", 7)
        parabolic_threshold = self._params.get("parabolic_threshold", 0.25)
        ...
        candidates: list[tuple] = []  # (symbol, score, dollar_volume)
        blocked_log = []
