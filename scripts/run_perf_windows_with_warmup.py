#!/usr/bin/env python3
"""
Run W1-W3 + FY2025 backtests on performance_bct (project 32034565).
Calculates warmup based on window length: 182 for FY2025, 150 for short windows.

Usage:
  python3 scripts/run_perf_windows_with_warmup.py
"""
...
def calculate_window_days(start, end):
...
def submit_backtest(name, start, end, compile_id):
    ...
    ey, em, ed = end.split("-")
    window_days = calculate_window_days(start, end)
    
    # Determine warmup days: longer windows use 182, short windows use smaller
    if name == "FY2025":
        warmup_days = 182  # Full year, can use larger warmup
    elif window_days < 30:
        warmup_days = min(window_days * 2, 182)  # At most double window length
    else:
        warmup_days = 182
    
    print(f"  {name}: window={window_days} days, warmup={warmup_days} days")
    ...
    if not bt_id:
        print(f"  {1}: no backtestId — {json.dumps(r)[:200]}")
        return None
