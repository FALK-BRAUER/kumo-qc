#!/usr/bin/env python3
"""
Run W1-W6 + FY2025 backtests sequentially (one at a time).
Poll until each completes, print results, then submit next window.
"""
...
def poll_backtest(name, bt_id):
    for i in range(60):  # 60 * 15 = 900 seconds (15 minutes)
        time.sleep(15)
...
def main():
    # Delete any existing perf-W* backtests to free nodes
    r = qc_post('/backtests/list', {'projectId': PROJECT_ID})
    for bt in r.get('backtests', []):
        bt_id = bt.get('backtestId')
        name = bt.get('name')
        if name.startswith('perf-W') or name.startswith('perf-FY2025') or name.startswith('perf-baseline'):
