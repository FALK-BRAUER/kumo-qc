...
if len(sys.argv) != 2:
    print("Usage: python3 poll_single.py <backtestId>")
    exit(1)
...
if resp.get('success'):
    ...
    error = bt.get('error')
    state = bt.get('state')
    print(f"{bt_id} {name} progress={progress} completed={completed} error={error} state={state}")
    if completed:
        sharpe = bt.get('statistics', {}).get('Sharpe Ratio', 'N/A')
        net_profit = bt.get('statistics', {}).get('Total Net Profit', 'N/A')
        orders = bt.get('statistics', {}).get('Total Orders', 'N/A')
        trades = bt.get('statistics', {}).get('Total Trades', 'N/A')
        print(f"Sharpe={sharpe}, NetProfit={net_profit}, Orders={orders}, Trades={trades}")
else:
    print("Failed:", resp)