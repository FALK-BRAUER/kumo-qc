#!/usr/bin/env python3
"""
Submit a single window backtest, poll, delete if stuck.
"""
...
def poll_backtest(name, bt_id):
    print(f"  Polling {name} backtest {bt_id[:16]}...")
...
def main():
    name = sys.argv[1] if len(sys.argv) > 1 else 'W1'
    start = sys.argv[2] if len(sys.argv) > 2 else '2026-04-07'
    end = sys.argv[3] if len(sys.argv) > 3 else '2026-04-11'
    ...
    if bt_id:
        ...
        print(f"{name}: submit failed (no spare nodes)")
        print("Check capacity via scripts/cleanup_running_backtests.py")
