#!/usr/bin/env python3
"""
Run W1-W3 backtests sequentially on performance_bct (project 32034565).
Only submits next window after previous completes.

Usage:
  python3 scripts/run_perf_windows_sequential.py
"""
...
def delete_auto_backtest():
    ...
    for bt in bt_list:
        ...
        name = bt.get('name')
        # Delete any running backtest
        if bt_id and bt.get('status') != 'Completed':
...
def wait_for_capacity(name):
    """Wait for capacity by checking if any backtests are running."""
    print(f"  Waiting for capacity for {name}...")
    for _ in range(30):
        backtests = qc_post('/backtests/read', {'projectId': PROJECT_ID})
        bt_list = backtests.get('backtests', [])
        running = sum(1 for bt in bt_list if bt.get('status') != 'Completed')
        if running == 0:
            return True
        print(f"  {name}: {running} backtests running, waiting...")
        time.sleep(10)
    print(f"  {name}: capacity not available after 5 minutes")
    return False
...
def main():
    ...
    for name, start, end in WINDOWS:
        ...
        if bt_id:
            ...
            print(f"  {name}: submission failed")
