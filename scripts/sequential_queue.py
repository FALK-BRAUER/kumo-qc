...
def main():
    # current backtest ID
    current_id = "31d515fee5ff816cf8cb060dd1bc9ac8"
    ...
    end_date = "2026-05-16"
    next_id = compile_and_submit("perf-W6", start_date, end_date, 200)
    if next_id:
        print(f"Submitted W6, backtest ID: {next_id}")
        # Poll W6 until completion
        while True:
            bt = poll_backtest(next_id)
            if bt is None:
                break
            if bt.get('completed'):
                print(f"W6 completed with error={bt.get('error')}")
                break
            time.sleep(30)

    # FY2025 will be submitted later after debugging ETF exit loop crash
    # submit_fy2025.py uses warmup_days=750
