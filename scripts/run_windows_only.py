#!/usr/bin/env python3
"""
Run W1-W6 backtests only (skip FY2025 and baseline-fixed).
...
def poll_backtest(name, bt_id):
    ...
    print(f"  {name}: TIMEOUT — deleting stuck backtest")
    try:
...
def main():
    ...
    for name, start, end in WINDOWS:
        ...
        if bt_id:
            ...
            # capacity issue — wait 30 seconds and retry
            print(f"  {name}: waiting 30 seconds...")
            time.sleep(30)
            bt_id = submit_backtest(name, start, end, compile_id)
            if bt_id:
                result = poll_backtest(name, bt_id)
                ...
                results.append({'name': name, 'error': 'capacity'})
        time.sleep(2)
    ...
    print("-" * 60)
    for r in results:
        net_profit = r.get('net_profit', 'ERROR')
        cagr = r.get('cagr', 'ERROR')
        sharpe = r.get('sharpe', 'ERROR')
        trades = r.get('trades', 'ERROR')
        print(f"{r['name']:<10} {net_profit:<14} {cagr:<12} {sharpe:<10} {trades}")
    print("=" * 60)
