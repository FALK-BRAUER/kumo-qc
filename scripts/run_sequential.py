#!/usr/bin/env python3
"""
Run windows sequentially, delete previous backtest before submitting next.
"""
import json, hashlib, time, base64, subprocess, sys, urllib.request
...
WINDOWS = [
    ...
    ("baseline-fixed", "2020-01-01", "2026-12-31"),
]
...
def delete_all_backtests():
    ...
    for bt in r.get('backtests', []):
        ...
        name = bt.get('name')
        print(f"Deleting {bt_id[:16]}... {name}")
        qc_post('/backtests/delete', {'projectId': PROJECT_ID, 'backtestId': bt_id})

def submit_and_wait(name, start, end, compile_id):
    ...
    if not bt_id:
        ...
        return {'name': name, 'error': 'submit_failed'}
...
def main():
    ...
    for name, start, end in WINDOWS:
        print(f"\n{name} ({start} → {end})")
        result = submit_and_wait(name, start, end, compile_id)
        results.append(result)
        # Delete after completion
        if result.get('error') not in ['submit_failed', 'timeout']:
            delete_all_backtests()
        time.sleep(2)
