...
- **A.2 — cloud-smoke gate BEFORE any FY.** Before ANY real cloud FY, run `python3 scripts/qc_v2_cloud.py smoke` — a SHORT (≈2-day) post-warmup cloud BT that exercises the construction paths (intraday/weekly/daily seed), gated on `assert_cloud_clean`. Catches an interop crash in **minutes** instead of a false-green FY. This is the gate that would have caught #313 immediately.
