...
class BctEngineAlgorithm(QCAlgorithm):  # pragma: no cover - QC runtime
    ...
    # trimmed below the weekly floor. Trim WARMUP_DAYS freely for the DAILY indicators (sma200=200tr);
    # the weekly comes from the cache, with this floor as the fail-loud/fallback computability test.
    ...
    def _weekly_scalars_for(self, sym: Any, asof_date: Any) -> dict[str, float] | None:
        ...
        if fp:
            ...
            # #368 DIAGNOSTIC (no-commit): log each miss (sym, asof_date, whether sym had ANY cached
            # rows) → enumerate the 4106 → classify late-listed-insufficient (correct drop) vs
            # true-cache-gap (the trimmed-warmup divergence risk).
            if getattr(self, "WEEKLY_MISS_LOG", False):
                _had = "sym_cached" if (self._weekly_loaded.get(key) is not None) else "sym_absent"
                self.log(f"WEEKLY_MISS|{key}|{asof_date}|{_had}")
