...
tag schema, and pure runtime helpers such as `watchlist_carry.py`. Does not go here: offline/runner
mechanics (those live in `sweeps/` + `scripts/`, NOT bundled), phase implementations
(`src/phases/`), or the engine core (`src/engine/`).
