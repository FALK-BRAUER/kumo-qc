...
- **What's here:** `base.py` (PhaseInterface Protocol + BasePhase + result types), `engine.py` (StrategyEngine with PHASE_ORDER scheduling and upstream/downstream contract validation), `context.py` (BarState, PhaseContext, OrderIntent dataclasses), `config.py` (StrategyConfig, RuntimeConfig, and Slot wiring), `logger.py` (ComponentLogger for JSON-lines telemetry).
