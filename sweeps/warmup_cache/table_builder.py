...
SCALAR_FIELDS = (
    ...
    # the SNAPSHOT capture reads the daily KIJUN. Both appended (NOT in the first-14 scoring set →
    # score_symbol_cached unaffected) so the daily-clock exit + snapshot consumers can be cache-fed too,
    # enabling the full set_warmup skip.
    "d_cloud_bottom",
    "d_kijun",
)
    ...
    *, adx_period: int = 9,
    ...
    for d, o, h, l, c in ((b[0], b[1], b[2], b[3], b[4]) for b in bars):
        ...
        yield d, {
            ...
            "d_kijun": d_ichi.kijun,  # #358b: the snapshot capture's daily_kijun
        }
