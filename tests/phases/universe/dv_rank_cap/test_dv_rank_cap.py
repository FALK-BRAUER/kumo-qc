"""Phase tests for DvRankCap — the rank+cap universe consumer (#220 / #238).

Constructor: DvRankCap(DvRankCap.Params(...), logger=None). Reads the LIVE-selected ranked
order `qc._ranked_today` (list[str], lowercase, DV-desc rank) and emits ranked_candidates
preserving rank order ∩ active. The filter→rank+cap MATH lives in
runtime.universe_select.select_live_universe (run live by lean_entry); these cover the
consumer contract, RANK-ORDER PRESERVATION (the #182 fix), and the fail-loud / no-raise
branches.
"""
