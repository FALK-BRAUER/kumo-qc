"""
v2-delta: constructor is VixIchimokuTier(VixIchimokuTier.Params(...), logger=None).

NOTE: the source tests passed params={"max_positions": N} and asserted the tier-1
cap == N. `max_positions` is a FORBIDDEN charter count-cap param and is NOT a Params
field in v2 — the tier-1 fallback is the fixed oracle default (9999). The tier LOGIC
(tier 2 above cloud, tier 1 below) is unchanged; the cap-value assertions now check
the verbatim default 9999. champion-asis-v1 never overrode it, so behavior is identical.
"""
...
def test_vix_above_cloud_top_sets_tier2_unlimited():
    ...
    phase = VixIchimokuTier(VixIchimokuTier.Params(), logger=None)
    ctx = make_ctx(qc)
...
def test_vix_below_cloud_top_sets_tier1():
    ...
    # tier-1 fallback cap is the fixed oracle default (max_positions param removed in v2)
    assert vix_tier["max_positions"] == 9999
...
def test_gate_disabled_passes_with_default_capacity():
    ...
    assert result.blocked is False
    vix_tier = ctx.bar_state.phase_outputs.get("vix_tier", [{}])[-1]
    # gate disabled → fixed oracle default capacity
    assert vix_tier["max_positions"] == 9999
