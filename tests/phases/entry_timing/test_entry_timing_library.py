"""Tests for the entry_timing catalog (phases.entry_timing.library, ADR D3).
...
def test_catalog_is_a_tuple_of_classes_not_strings() -> None:
    assert isinstance(ENTRY_TIMING_PHASES, tuple)
    for impl in ENTRY_TIMING_PHASES:
        assert isinstance(impl, type), "catalog holds class refs, never name strings"


def test_market_on_open_entry_is_catalogued() -> None:
    assert MarketOnOpenEntry in ENTRY_TIMING_PHASES


def test_every_catalogued_phase_declares_entry_timing_kind() -> None:
    for impl in ENTRY_TIMING_PHASES:
        assert impl.PHASE_KIND == "entry_timing"
...
def test_every_catalogued_phase_exposes_space_and_complexity() -> None:
    for impl in ENTRY_TIMING_PHASES:
        ...
        complexity.validate(space)


def test_market_on_open_entry_space_is_empty_baseline() -> None:
